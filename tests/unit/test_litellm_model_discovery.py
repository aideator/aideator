...
class TestLiteLLMModelDiscovery:
    ...
    def test_create_model_info_anthropic(self):
        ...
        model_info = self.discovery_service._create_model_info(
        ...
        assert model_info["category"] == "general"
        assert (
            model_info["description"] == "Balanced performance and cost from Anthropic"
        )
    ...
    def test_deduplicate_models(self):
        ...
        claude_model = next(
            m for m in unique_models if m["model_name"] == "claude-3-opus"
        )
    ...
    def test_generate_display_name_basic(self):
        ...
        assert (
            self.discovery_service._generate_display_name("claude-3-sonnet")
            == "Claude 3 Sonnet"
        )
        assert (
            self.discovery_service._generate_display_name("gemini-1.5-pro")
            == "Gemini 1.5 Pro"
        )
    ...
    def test_generate_display_name_with_prefixes(self):
        ...
        assert (
            self.discovery_service._generate_display_name("anthropic/claude-3-opus")
            == "Claude 3 Opus"
        )
        assert (
            self.discovery_service._generate_display_name("groq/llama3-8b-8192")
            == "Llama3 8b 8192"
        )
    ...
    def test_generate_display_name_special_terms(self):
        ...
        assert (
            self.discovery_service._generate_display_name("text-embedding-ada-002")
            == "Text Embedding Ada 002"
        )
        assert self.discovery_service._generate_display_name("tts-1-hd") == "TTS 1 Hd"
        assert (
            self.discovery_service._generate_display_name("whisper-v1") == "Whisper V1"
        )

    def test_determine_category_embedding(self):
        """Test category determination for embedding models."""
        assert (
            self.discovery_service._determine_category(
                "text-embedding-3-large", "openai"
            )
            == "embedding"
        )
        assert (
            self.discovery_service._determine_category("embed-english-v3.0", "cohere")
            == "embedding"
        )
    ...
    def test_determine_category_vision(self):
        ...
        assert (
            self.discovery_service._determine_category("gpt-4-vision-preview", "openai")
            == "vision"
        )
        assert (
            self.discovery_service._determine_category("gpt-4o-multimodal", "openai")
            == "vision"
        )
    ...
    def test_determine_category_code(self):
        ...
        assert (
            self.discovery_service._determine_category("codex-mini", "openai") == "code"
        )
        assert (
            self.discovery_service._determine_category("deepseek-coder", "deepseek")
            == "code"
        )
    ...
    def test_determine_category_speech(self):
        ...
        assert (
            self.discovery_service._determine_category("whisper-large-v3", "openai")
            == "speech-to-text"
        )
        assert (
            self.discovery_service._determine_category("tts-1", "openai")
            == "text-to-speech"
        )
    ...
    def test_determine_category_advanced(self):
        ...
        assert (
            self.discovery_service._determine_category("gpt-4o", "openai") == "advanced"
        )
        assert (
            self.discovery_service._determine_category("claude-3-opus", "anthropic")
            == "advanced"
        )
        assert (
            self.discovery_service._determine_category("gemini-pro", "gemini")
            == "advanced"
        )
        assert (
            self.discovery_service._determine_category("grok-beta", "xai") == "advanced"
        )

    def test_determine_category_general(self):
        """Test category determination for general models."""
        assert (
            self.discovery_service._determine_category("gpt-3.5-turbo", "openai")
            == "general"
        )
        assert (
            self.discovery_service._determine_category("llama2-7b", "meta") == "general"
        )
    ...
    def test_get_api_key_env_var(self):
        ...
        assert (
            self.discovery_service._get_api_key_env_var("text-completion-openai")
            == "OPENAI_API_KEY"
        )
        assert (
            self.discovery_service._get_api_key_env_var("anthropic")
            == "ANTHROPIC_API_KEY"
        )
        ...
        assert (
            self.discovery_service._get_api_key_env_var("deepseek")
            == "DEEPSEEK_API_KEY"
        )
        assert (
            self.discovery_service._get_api_key_env_var("vertex_ai")
            == "GOOGLE_APPLICATION_CREDENTIALS"
        )
        assert (
            self.discovery_service._get_api_key_env_var("bedrock")
            == "AWS_ACCESS_KEY_ID"
        )
    ...
    def test_supports_function_calling(self):
        ...
        assert (
            self.discovery_service._supports_function_calling("gpt-3.5-turbo") is True
        )
        assert (
            self.discovery_service._supports_function_calling("claude-3-opus") is True
        )
        ...
        assert (
            self.discovery_service._supports_function_calling("text-embedding-ada-002")
            is False
        )
    ...
    def test_supports_vision(self):
        ...
        assert (
            self.discovery_service._supports_vision("text-embedding-ada-002") is False
        )
    ...
    def test_integration_complete_flow(self, mock_litellm):
        ...
        assert len(models) == 16  # Total unique models
        ...
        providers = {m["litellm_provider"] for m in models}
        ...
        embedding_model = next(
            (m for m in models if "embedding" in m["model_name"]), None
        )
