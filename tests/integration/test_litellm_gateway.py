...
class TestLiteLLMGateway:
    ...
    async def test_gateway_model_routing(self, openai_client, test_models):
        ...
        for model in test_models:
            response = openai_client.chat.completions.create(
                model=model,
                messages=[{"role": "user", "content": "What model are you? Answer with just the model name."}],
                max_tokens=20,
                temperature=0.1
            )
            ...
            if ("gpt" in model and "gpt" in content) or ("claude" in model and "claude" in content) or ("gemini" in model and "gemini" in content):
