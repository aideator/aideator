...
class ModelDefinitionDB(SQLModel, table=True):
    ...
    model_name: str = Field(
        unique=True,
        index=True,
        description="LiteLLM model name (e.g., 'gpt-4', 'claude-3-opus')",
    )
    ...
    description: str | None = Field(
        default=None, description="Detailed description for UI"
    )
    ...
    input_price_per_1m_tokens: float | None = Field(
        default=None, description="Cost per 1M input tokens in USD"
    )
    output_price_per_1m_tokens: float | None = Field(
        default=None, description="Cost per 1M output tokens in USD"
    )
    input_cost_per_token: float | None = Field(
        default=None, description="Cost per input token in USD"
    )
    output_cost_per_token: float | None = Field(
        default=None, description="Cost per output token in USD"
    )
    ...
    requires_region: bool = Field(
        default=False, description="Whether a region is required"
    )
    requires_project_id: bool = Field(
        default=False, description="Whether a project ID is required"
    )
    ...
    category: str | None = Field(
        default=None, description="Category like 'general', 'code', 'vision'"
    )
    tags: list[str] = Field(default_factory=list, sa_column=Column(JSON))
    is_recommended: bool = Field(
        default=False, description="Whether we recommend this model"
    )
    is_popular: bool = Field(
        default=False, description="Whether this is a popular model"
    )

    # Status and tracking
    is_active: bool = Field(
        default=True, description="Whether this model is currently available"
    )
