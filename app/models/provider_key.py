...
class ProviderAPIKeyDB(SQLModel, table=True):
    ...
    # Primary key
    id: str = Field(
        primary_key=True, description="Unique identifier, format: provkey_xxx"
    )
    ...
    provider: str = Field(
        nullable=False, index=True, description="Provider name: openai, anthropic, etc"
    )
    model_name: str | None = Field(
        default=None, index=True, description="Optional: specific model override"
    )

    # Encrypted storage
    encrypted_key: str = Field(nullable=False, description="Fernet-encrypted API key")
    key_hint: str = Field(
        nullable=False, description="Last 4 chars for UI display: ...abc123"
    )
    encryption_version: int = Field(
        default=1, description="Encryption algorithm version"
    )

    # Metadata
    name: str | None = Field(
        default=None, description="User-friendly name for this key"
    )
    ...
    last_error: str | None = Field(
        default=None, description="Last error message if validation failed"
    )
...
class ProviderAPIKeyAuditLog(SQLModel, table=True):
    ...
    provider_key_id: str = Field(
        foreign_key="provider_api_keys.id", nullable=False, index=True
    )

    action: str = Field(
        nullable=False, description="Action: created, updated, deleted, used, validated"
    )
