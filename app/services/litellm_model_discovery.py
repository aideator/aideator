...
class LiteLLMModelDiscovery:
    ...
    def get_all_supported_models(self) -> List[Dict[str, any]]:
        ...
        self.discovered_models = {model['model_name']: model for model in unique_models}
        self.last_discovery_time = datetime.utcnow()
        
        logger.info(f"Discovered {len(unique_models)} unique models from {len(litellm.models_by_provider)} providers")
    ...
    def _deduplicate_models(self, models: List[Dict]) -> List[Dict]:
        ...
        models_sorted = sorted(models, key=lambda m: provider_priority.get(m['litellm_provider'], 100))
    ...
    def _generate_display_name(self, model_name: str) -> str:
        ...
        name = model_name
        prefixes = ['openai/', 'anthropic/', 'vertex_ai/', 'bedrock/', 'azure/', 'groq/', 'mistral/', 'deepseek/', 'xai/', 'gemini/', 'cohere/', 'perplexity/', 'fireworks_ai/']
        ...
        formatted_parts = []
        for part in parts:
            if part.lower() in ['gpt', 'ai', 'api', 'llm', 'tts', 'stt']:
                formatted_parts.append(part.upper())
            elif part.lower() in ['v1', 'v2', 'v3', 'v4', 'v5']:
                formatted_parts.append(part.upper())
            else:
    ...
    def _generate_description(self, model_name: str, provider: str) -> str:
        ...
        elif 'vision' in model_name.lower() or 'multimodal' in model_name.lower():
        ...
        elif 'moderation' in model_name.lower():
    ...
    def _determine_category(self, model_name: str, provider: str) -> str:
        ...
        if 'embed' in model_name.lower():
            ...
            return 'moderation'
        elif any(x in model_name.lower() for x in ['gpt-4', 'claude-3-opus', 'claude-3-5', 'gemini-pro', 'grok']):
            return 'advanced'
        else:
            return 'general'
    ...
    def _generate_tags(self, model_name: str, provider: str) -> List[str]:
        ...
        if any(x in model_name.lower() for x in ['gpt-4', 'opus', 'pro', 'grok', 'reasoner']):
    ...
    def _is_recommended(self, model_name: str) -> bool:
        ...
        recommended = [
            'gpt-4o', 'gpt-4', 'claude-3-opus', 'claude-3-sonnet', 'claude-3-5-sonnet',
            'gemini-1.5-pro', 'gemini-1.5-flash', 'grok-beta', 'deepseek-reasoner'
        ]
    ...
    def _is_popular(self, model_name: str) -> bool:
        ...
        popular = [
            'gpt-4', 'gpt-3.5-turbo', 'claude-3', 'gemini', 'grok', 'deepseek'
        ]
    ...
    def _supports_function_calling(self, model_name: str) -> bool:
        ...
        function_models = [
            'gpt-4', 'gpt-3.5-turbo', 'claude-3', 'gemini', 'grok'
        ]
        return any(f in model_name.lower() for f in function_models)
    
    def _supports_vision(self, model_name: str) -> bool:
        """Determine if the model supports vision."""
        return 'vision' in model_name.lower() or any(v in model_name.lower() for v in [
            'gpt-4o', 'gpt-4-vision', 'claude-3-opus', 'claude-3-sonnet', 'claude-3-5-sonnet',
            'gemini-pro-vision', 'gemini-1.5', 'gemini-2.0', 'grok-vision'
        ])
