...
from app.core.dependencies import get_current_user, OptionalCurrentUser
...
async def github_callback(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not primary_email:
            primary_email = github_user.get("email", f"{github_user['login']}@github.local")
    ...
    if not user:
        ...
        result = await db.execute(
            select(User).where(col(User.email) == primary_email)
        )
        user = result.scalar_one_or_none()
