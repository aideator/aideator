...
async def get_job_keys(
    ...
    try:
        ...
        logger.info(f"🔑 Job key request for run {run_id}, variation {variation_id}, user {user_id}")
        ...
        for key in user_keys:
            if key.is_active:
                try:
                    decrypted_key = await provider_service.decrypt_api_key(db, user, key.id)
                    provider_name = key.provider.upper()
                    decrypted_keys[f"{provider_name}_API_KEY"] = decrypted_key
                    logger.info(f"✅ Retrieved {provider_name} API key for job {run_id}")
                except Exception as e:
                    logger.warning(f"❌ Failed to decrypt {key.provider} key for user {user_id}: {e}")
        ...
        if model_name:
            ...
            if model_key and model_key.is_active:
                try:
                    decrypted_key = await provider_service.decrypt_api_key(db, user, model_key.id)
                    provider_name = model_key.provider.upper()
                    decrypted_keys[f"{provider_name}_API_KEY"] = decrypted_key
                    logger.info(f"✅ Retrieved model-specific {provider_name} key for {model_name}")
                except Exception as e:
                    logger.warning(f"❌ Failed to decrypt model-specific key: {e}")
        
        expires_at = datetime.fromtimestamp(payload.get("exp", 0)).isoformat()
        
        logger.info(f"🎯 Successfully retrieved {len(decrypted_keys)} API keys for job {run_id}")
        
        return JobKeyResponse(
            keys=decrypted_keys,
            expires_at=expires_at
        )
        
    except jwt.ExpiredSignatureError:
        logger.warning(f"❌ Expired job token used")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
