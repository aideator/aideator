...
async def create_run(
    ...
    stream_url = (
        f"ws://localhost:{settings.port}{settings.api_v1_prefix}/ws/runs/{run_id}"
    )
...
@router.get(
    "/{run_id}/diffs",
    response_model=list[dict],
    summary="Get diffs for a run",
    description="""
    Get the diff information for all changed files in a run.

    Returns an array of objects with the structure:
    { oldFile: { name, content }, newFile: { name, content } }

    This endpoint retrieves the diffs that were generated and saved
    by the agent during code execution.
    """,
)
async def get_run_diffs(
    ...
    Returns the diff array that was generated by the agent
    ...
    # Build query for diff outputs
    diffs_query = select(AgentOutput).where(
        AgentOutput.run_id == run_id, AgentOutput.output_type == "diffs"
    )

    # Apply variation filter if specified
    if variation_id is not None:
        diffs_query = diffs_query.where(AgentOutput.variation_id == variation_id)

    # Order by timestamp (latest first)
    diffs_query = diffs_query.order_by(desc(AgentOutput.timestamp)).limit(1)

    # Execute query
    result = await db.execute(diffs_query)
    diff_output = result.scalar_one_or_none()

    if not diff_output:
        # Return empty array if no diffs found
        return []

    try:
        # Parse the JSON content
        return json.loads(diff_output.content)
    except json.JSONDecodeError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Invalid diff data format",
        )
