...
async def get_database_stats(
    ...
    recent_messages = await db.scalar(
        ...
        .where(AgentOutput.timestamp > yesterday)
    )
...
async def get_message_stream(
    ...
    query = query.order_by(AgentOutput.timestamp.desc()).offset(offset).limit(limit)
...
async def search_messages(
    ...
    search_query = select(AgentOutput).where(
        AgentOutput.content.ilike(f"%{query}%")
    )
    ...
    search_query = search_query.order_by(AgentOutput.timestamp.desc()).limit(limit)
    ...
    return [
        {
            ...
            "timestamp": msg.timestamp.isoformat(),
            "output_type": msg.output_type,
            "match_preview": msg.content[:200] + "..." if len(msg.content) > 200 else msg.content,
        }
        for msg in messages
    ]
...
async def cleanup_database(
    ...
    messages_count = await db.scalar(
        select(func.count(AgentOutput.id))
        .where(AgentOutput.run_id.in_(old_run_ids))
    )
...
async def database_health(
    ...
    try:
        ...
        # Test read
        count = await db.scalar(select(func.count(Run.id)))
