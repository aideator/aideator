...
def upgrade() -> None:
    ...
    # Create indexes
    op.create_index(
        "idx_provider_key_lookup",
        "provider_api_keys",
        ["user_id", "provider", "is_active"],
        unique=False,
    )
    op.create_index(
        "idx_provider_key_model",
        "provider_api_keys",
        ["user_id", "model_name", "is_active"],
        unique=False,
    )
    op.create_index(
        op.f("ix_provider_api_keys_provider"),
        "provider_api_keys",
        ["provider"],
        unique=False,
    )
    op.create_index(
        op.f("ix_provider_api_keys_user_id"),
        "provider_api_keys",
        ["user_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_provider_api_keys_model_name"),
        "provider_api_keys",
        ["model_name"],
        unique=False,
    )
    op.create_index(
        op.f("ix_provider_api_keys_is_active"),
        "provider_api_keys",
        ["is_active"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_provider_api_key_audit_logs_user_id"),
        "provider_api_key_audit_logs",
        ["user_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_provider_api_key_audit_logs_provider_key_id"),
        "provider_api_key_audit_logs",
        ["provider_key_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_provider_api_key_audit_logs_provider_key_id"),
        table_name="provider_api_key_audit_logs",
    )
    op.drop_index(
        op.f("ix_provider_api_key_audit_logs_user_id"),
        table_name="provider_api_key_audit_logs",
    ...
    op.drop_index(
        op.f("ix_provider_api_keys_is_active"), table_name="provider_api_keys"
    )
    op.drop_index(
        op.f("ix_provider_api_keys_model_name"), table_name="provider_api_keys"
    )
